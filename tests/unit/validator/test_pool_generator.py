    ...
    def test_generate_assets_and_pools(self):
        ...
        for i in range(0, 100):
            ...
            for pool_id, pool_info in result["pools"].items():
                ...
                self.assertTrue(MIN_BASE_RATE <= pool_info["base_rate"] <= MAX_BASE_RATE)
                ...
                self.assertTrue(MIN_KINK_SLOPE <= pool_info["kink_slope"] <= MAX_KINK_SLOPE)

                self.assertEqual(pool_info["optimal_util_rate"], OPTIMAL_UTIL_RATE)

                self.assertTrue("borrow_amount" in pool_info)
                self.assertTrue(MIN_BORROW_AMOUNT <= pool_info["borrow_amount"] <= MAX_BORROW_AMOUNT)
